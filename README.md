...
An automated streaming overlay for *Age of Empires II: Definitive Edition* of Sun Tzu.
The application
- captures your game window on a configurable timer or press of a hotkey,
- sends the screenshot to a specified Vision LLM,
- and pushes a releavnt quote to an OBS Browser Source.
...
On first launch a `config.json` is created with sensible defaults.
...
The GUI is organised into tabs. Every tab has a **Save** button that persists the current values to `config.json`.
...
| No-Repeat History (N) | How many recent quotes to remember and suppress repeats for (persisted across restarts via `quote_history.json`) |
...
| Port 5000 in use | A warning dialog will appear. Another instance of the app may already be running. |
| Hotkey not working | On Windows the `keyboard` library requires the app to run **as Administrator** for system-wide hotkey detection. Right-click the terminal or shortcut and choose "Run as administrator". |
...
- `config.json` stores your API key in **plaintext** on disk. Do not share this file or commit it to source control. It is excluded from git via `.gitignore`.
...
- **Window capture area:** The app captures the full window bounding rect rather than just the DirectX client surface. In windowed (non-fullscreen) mode this includes the Windows title bar. Use Borderless Window mode for best results.
...
```
aoe2de-suntzu-ai/
├── assets/                  # Overlay image assets (sun-tzu-background.png)
├── backend/
...
├── tests/
│   ├── assets/              # Test fixture images (sample.jpg)
│   ├── unit/
│   └── integration/
├── requirements.txt
└── README.md
